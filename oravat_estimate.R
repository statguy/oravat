...
oravat.scaled <- oravat.raw %>%
  dplyr::mutate_each(funs(xscale), marmar1km, marmarPrevYear, marmar2YearsAgo, kernel_new, kernel_lagged, interpolated_conevalue, triangletype, xcoord, ycoord)
...
# Construct mesh
mesh <- SpaceTimeModels::NonConvexHullMesh$new(knots = oravat@sp, knotsScale = 1/coord.scale, cutoff = 3e4, maxEdge = c(5e4, 6e5), offset = c(1e4, 1e6), innerConvex = 0.05, outerConvex = 1.5)
mesh$getINLAMesh()$n
...
save(model1, file = "model1-12.RData")
model1$summary()
